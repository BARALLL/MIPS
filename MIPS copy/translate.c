...
void getOpCode(instr* inst)
{
	...
	char* instrNameRTab[] = { "ADD", "AND", "DIV", "JR", "MFHI", "MFLO", "MULT", "OR", "ROTR", "SLL", "SLT", "SRL", "SUB", "SYSCALL", "XOR"};
	uint8_t instrCodeRTab[] = { 0b100000, 0b100100, 0b011010, 0b001000, 0b010000, 
...
void makeMIPSInstruction(instr* inst, uint8_t rd, uint8_t rs, uint8_t rt)
{
	...
	case 'R':
		compInst = 0b000000;
		compInst = (compInst << 5) & rs & 0b11111;
		compInst = (compInst << 5) & rt & 0b11111;
		compInst = (compInst << 5) & rd & 0b11111;
		compInst = (compInst << 5) & opcode & 0b111111;
...
void getParam(uint32_t inst, uint8_t* rs, uint8_t* rd, uint8_t* rt)
{
	...
	*rs = (uint8_t)((inst >> 21) & (uint8_t)0b00011111);
	*rt = (uint8_t)((inst >> 16) & (uint8_t)0b00011111);
	*rd = (uint8_t)((inst >> 11) & (uint8_t)0b00011111);
}
...
int getFullInstruction(instr* inst) {
	// D�claration de l'instruction ADD en langage assembleur MIPS.
	// Cette instruction ajoute le contenu des registres $s1 et $s2 et stocke le r�sultat dans $s3.
	char* instStr = inst->inst;

	// Extraction des arguments des registres de l'instruction en tant que cha�ne de caract�res.
	// Nous utilisons la fonction strtok pour d�couper la cha�ne en diff�rents tokens en utilisant l'espace comme s�parateur.
	char* name = strtok(instStr, " ");  // R�cup�re "ADD"
	char* rs = strtok(NULL, " ");  // R�cup�re "$s1"
	char* rt = strtok(NULL, " ");  // R�cup�re "$s2"
	char* rd = strtok(NULL, " ");  // R�cup�re "$s3"

	// Conversion des arguments des registres en uint32_t.
	// Nous utilisons la fonction strtol pour convertir les cha�nes de caract�res en entiers.
	...
	makeMIPSInstruction(inst, rs, rd, rt);
}